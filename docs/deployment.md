...
On September 20, 2026, `npm run test:live` passed all eight scenarios through the public URL in 23.1 seconds, using normal HTTPS certificate validation. Verification covered registration; installation creation, editing, deletion, and persistence after page reload; reading ingestion with a valid zero value; alert resolution; logout and login; demo access; monitoring/date filters; desktop/mobile layouts; and retry states. Browser assertions confirmed Secure, HTTP-only, SameSite=Lax session cookies. The test deleted its own installation and left one empty verification account with a random password. Readings remain clearly labeled as simulated and historical.

Earlier checks found HTTP 403 **“HTTPS is required”** on registration. The owner reported deployment changes, and the complete public account flow now passes. The exact private proxy configuration has not been inspected from this workspace, so the precise change is not recorded as verified. The incident demonstrates why browser HTTPS alone does not establish what protocol Express sees after proxies.

If that symptom recurs, inspect the connector-to-Caddy-to-app path and the deployed `TRUST_PROXY`. Caddy ignores incoming forwarded headers by default; when an authenticated/private tunnel connector precedes it, configure Caddy's `servers > trusted_proxies` for that connector's actual IP/CIDR so the original HTTPS scheme is preserved. Review the real path before choosing Express's hop count. Validate and reload the existing Caddy configuration while preserving the portfolio's other routes. Keep the application's HTTPS check enabled. See [Caddy forwarded-header behavior](https://caddyserver.com/docs/caddyfile/directives/reverse_proxy#defaults) and [Express proxy settings](https://expressjs.com/en/guide/behind-proxies/).

After deployment changes, run `npm run test:live`. It reuses the eight application browser scenarios, creates one test account, exercises logout/login, and deletes that account's test installation. Account deletion is not implemented, so a successful run leaves an empty verification account with a random password. It performs no database reset, restart, or rollback. Rehearse recovery on the actual host separately before tagging a verified release.
