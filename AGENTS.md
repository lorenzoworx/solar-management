...
Current checkpoint: 7. Public demo: https://boywithabot.com/projects/solar-management/. Seven public read-only browser scenarios pass; registration returns 403 "HTTPS is required" and needs the actual proxy configuration/access to diagnose and fix. Do not disable HTTPS checks. Container recovery passes in CI; on-host recovery remains unverified. Support APP_BASE_PATH at build time and keep the public proxy stripping that prefix. Learning review remains deferred to the local, Git-ignored questions.md; do not commit that file or mark review complete without evidence.
